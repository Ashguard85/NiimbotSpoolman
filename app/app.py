...
def migrate():
    with conn() as c:
        ...
        ensure_column(c,'templates','qr_render_mode',"TEXT NOT NULL DEFAULT 'local'")
        ensure_column(c,'templates','qr_margin_modules',"INTEGER NOT NULL DEFAULT 1")
        ensure_column(c,'templates','qr_ec_level',"TEXT NOT NULL DEFAULT 'M'")
        ensure_column(c,'templates','text_size_mode',"TEXT NOT NULL DEFAULT 'auto'")
        ensure_column(c,'templates','text_font_size',"INTEGER NOT NULL DEFAULT 16")
        ensure_column(c,'templates','text_wrap_mode',"TEXT NOT NULL DEFAULT 'wrap'")
        ensure_column(c,'templates','text_rotation',"INTEGER NOT NULL DEFAULT 0")
        ensure_column(c,'templates','text_padding',"INTEGER NOT NULL DEFAULT 8")
        ...
        c.execute("INSERT INTO meta(key,value) VALUES('schema_version','10') ON CONFLICT(key) DO UPDATE SET value='10'")
...
def templates():
    ...
    row={'id':tid,'name':str(d.get('name') or 'Vorlage')[:120],'format':str(d.get('format') or '40x40')[:20],'line1':str(d.get('line1') or '')[:500],'line2':str(d.get('line2') or '')[:500],'line3':str(d.get('line3') or '')[:500],'qr_mode':str(d.get('qrMode') or d.get('qr_mode') or 'url')[:30],'qr_template':str(d.get('qrTemplate') or d.get('qr_template') or '{{spool.url}}')[:1000],'created_at':d.get('created_at') or t,'updated_at':t,'text_template':str(d.get('textTemplate') or d.get('text_template') or '\n'.join(x for x in [d.get('line1',''),d.get('line2',''),d.get('line3','')] if x))[:5000],'layout_mode':str(d.get('layoutMode') or d.get('layout_mode') or 'auto')[:20],'qr_size_mode':str(d.get('qrSizeMode') or d.get('qr_size_mode') or 'auto')[:20],'qr_width_percent':int(d.get('qrWidthPercent') or d.get('qr_width_percent') or 60),'text_align_mode':str(d.get('textAlignMode') or d.get('text_align_mode') or 'auto')[:20],'qr_render_mode':str(d.get('qrRenderMode') or d.get('qr_render_mode') or 'local')[:20],'qr_margin_modules':max(0,min(4,int(d.get('qrMarginModules') if d.get('qrMarginModules') is not None else d.get('qr_margin_modules',1)))),'qr_ec_level':str(d.get('qrEcLevel') or d.get('qr_ec_level') or 'M')[:2],'text_size_mode':str(d.get('textSizeMode') or d.get('text_size_mode') or 'auto')[:20],'text_font_size':max(8,min(30,int(d.get('textFontSize') or d.get('text_font_size') or 16))),'text_wrap_mode':str(d.get('textWrapMode') or d.get('text_wrap_mode') or 'wrap')[:20],'text_rotation':int(d.get('textRotation') if d.get('textRotation') is not None else d.get('text_rotation',0)),'text_padding':max(0,min(24,int(d.get('textPadding') if d.get('textPadding') is not None else d.get('text_padding',8))))}
    with conn() as c:
        c.execute('''INSERT INTO templates(id,name,format,line1,line2,line3,qr_mode,qr_template,created_at,updated_at,text_template,layout_mode,qr_size_mode,qr_width_percent,text_align_mode,qr_render_mode,qr_margin_modules,qr_ec_level,text_size_mode,text_font_size,text_wrap_mode,text_rotation,text_padding)
        VALUES(:id,:name,:format,:line1,:line2,:line3,:qr_mode,:qr_template,:created_at,:updated_at,:text_template,:layout_mode,:qr_size_mode,:qr_width_percent,:text_align_mode,:qr_render_mode,:qr_margin_modules,:qr_ec_level,:text_size_mode,:text_font_size,:text_wrap_mode,:text_rotation,:text_padding)
        ON CONFLICT(id) DO UPDATE SET name=excluded.name,format=excluded.format,line1=excluded.line1,line2=excluded.line2,line3=excluded.line3,qr_mode=excluded.qr_mode,qr_template=excluded.qr_template,text_template=excluded.text_template,layout_mode=excluded.layout_mode,qr_size_mode=excluded.qr_size_mode,qr_width_percent=excluded.qr_width_percent,text_align_mode=excluded.text_align_mode,qr_render_mode=excluded.qr_render_mode,qr_margin_modules=excluded.qr_margin_modules,qr_ec_level=excluded.qr_ec_level,text_size_mode=excluded.text_size_mode,text_font_size=excluded.text_font_size,text_wrap_mode=excluded.text_wrap_mode,text_rotation=excluded.text_rotation,text_padding=excluded.text_padding,updated_at=excluded.updated_at''',row)
    return jsonify(row)
...
def export():
    with conn() as c:
        return jsonify(format='spoolman-niimbot-backup',version=7,exported_at=now(),templates=[dict(r) for r in c.execute('SELECT * FROM templates')],history=[dict(r) for r in c.execute('SELECT * FROM history')])

@app.route('/api/import',methods=['POST','OPTIONS'])
def import_backup():
    if request.method=='OPTIONS':return ('',204)
    d=request.get_json(force=True) or {}
    if d.get('format')!='spoolman-niimbot-backup' or int(d.get('version',0)) not in (1,2,3,4,5,6,7):return jsonify(error='Ungültiges Backupformat'),400
    backup_db('pre-restore')
    with conn() as c:
        c.execute('DELETE FROM templates'); c.execute('DELETE FROM history')
        for raw in d.get('templates',[]):
            t=now(); row={'id':str(raw.get('id') or f"tpl-{int(time.time()*1000)}"),'name':str(raw.get('name') or 'Vorlage')[:120],'format':str(raw.get('format') or '40x40')[:20],'line1':str(raw.get('line1') or '')[:500],'line2':str(raw.get('line2') or '')[:500],'line3':str(raw.get('line3') or '')[:500],'qr_mode':str(raw.get('qr_mode') or raw.get('qrMode') or 'url')[:30],'qr_template':str(raw.get('qr_template') or raw.get('qrTemplate') or '{{spool.url}}')[:1000],'created_at':raw.get('created_at') or t,'updated_at':raw.get('updated_at') or t,'text_template':str(raw.get('text_template') or raw.get('textTemplate') or '\n'.join(x for x in [raw.get('line1',''),raw.get('line2',''),raw.get('line3','')] if x))[:5000],'layout_mode':str(raw.get('layout_mode') or raw.get('layoutMode') or 'auto')[:20],'qr_size_mode':str(raw.get('qr_size_mode') or raw.get('qrSizeMode') or 'auto')[:20],'qr_width_percent':int(raw.get('qr_width_percent') or raw.get('qrWidthPercent') or 60),'text_align_mode':str(raw.get('text_align_mode') or raw.get('textAlignMode') or 'auto')[:20],'qr_render_mode':str(raw.get('qr_render_mode') or raw.get('qrRenderMode') or 'local')[:20],'qr_margin_modules':max(0,min(4,int(raw.get('qr_margin_modules') if raw.get('qr_margin_modules') is not None else raw.get('qrMarginModules',1)))),'qr_ec_level':str(raw.get('qr_ec_level') or raw.get('qrEcLevel') or 'M')[:2],'text_size_mode':str(raw.get('text_size_mode') or raw.get('textSizeMode') or 'auto')[:20],'text_font_size':max(8,min(30,int(raw.get('text_font_size') or raw.get('textFontSize') or 16))),'text_wrap_mode':str(raw.get('text_wrap_mode') or raw.get('textWrapMode') or 'wrap')[:20],'text_rotation':int(raw.get('text_rotation') if raw.get('text_rotation') is not None else raw.get('textRotation',0)),'text_padding':max(0,min(24,int(raw.get('text_padding') if raw.get('text_padding') is not None else raw.get('textPadding',8))))}
            c.execute('''INSERT INTO templates(id,name,format,line1,line2,line3,qr_mode,qr_template,created_at,updated_at,text_template,layout_mode,qr_size_mode,qr_width_percent,text_align_mode,qr_render_mode,qr_margin_modules,qr_ec_level,text_size_mode,text_font_size,text_wrap_mode,text_rotation,text_padding) VALUES(:id,:name,:format,:line1,:line2,:line3,:qr_mode,:qr_template,:created_at,:updated_at,:text_template,:layout_mode,:qr_size_mode,:qr_width_percent,:text_align_mode,:qr_render_mode,:qr_margin_modules,:qr_ec_level,:text_size_mode,:text_font_size,:text_wrap_mode,:text_rotation,:text_padding)''',row)
