...
def migrate():
    with conn() as c:
        ...
        c.execute("INSERT INTO meta(key,value) VALUES('schema_version','7') ON CONFLICT(key) DO UPDATE SET value='7'")
...
def templates():
    ...
    with conn() as c:
        c.execute('''INSERT INTO templates(id,name,format,line1,line2,line3,qr_mode,qr_template,created_at,updated_at,text_template,layout_mode,qr_size_mode,qr_width_percent,text_align_mode,qr_render_mode,qr_margin_modules,qr_ec_level,text_size_mode,text_font_size)
        VALUES(:id,:name,:format,:line1,:line2,:line3,:qr_mode,:qr_template,:created_at,:updated_at,:text_template,:layout_mode,:qr_size_mode,:qr_width_percent,:text_align_mode,:qr_render_mode,:qr_margin_modules,:qr_ec_level,:text_size_mode,:text_font_size)
...
def export():
    with conn() as c:
        return jsonify(format='spoolman-niimbot-backup',version=6,exported_at=now(),templates=[dict(r) for r in c.execute('SELECT * FROM templates')],history=[dict(r) for r in c.execute('SELECT * FROM history')])

@app.route('/api/import',methods=['POST','OPTIONS'])
def import_backup():
    if request.method=='OPTIONS':return ('',204)
    d=request.get_json(force=True) or {}
    if d.get('format')!='spoolman-niimbot-backup' or int(d.get('version',0)) not in (1,2,3,4,5,6):return jsonify(error='Ungültiges Backupformat'),400
