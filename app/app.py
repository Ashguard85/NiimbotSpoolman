...
def migrate():
    with conn() as c:
        ...
        c.execute("INSERT INTO meta(key,value) VALUES('schema_version','2') ON CONFLICT(key) DO UPDATE SET value='2'")
...
def templates():
    ...
    row={'id':tid,'name':str(d.get('name') or 'Vorlage')[:120],'format':str(d.get('format') or '40x40')[:20],'line1':str(d.get('line1') or '')[:500],'line2':str(d.get('line2') or '')[:500],'line3':str(d.get('line3') or '')[:500],'qr_mode':str(d.get('qrMode') or d.get('qr_mode') or 'url')[:30],'qr_template':str(d.get('qrTemplate') or d.get('qr_template') or '{{spool.url}}')[:1000],'created_at':d.get('created_at') or t,'updated_at':t,'text_template':str(d.get('textTemplate') or d.get('text_template') or '\n'.join(x for x in [d.get('line1',''),d.get('line2',''),d.get('line3','')] if x))[:5000],'layout_mode':str(d.get('layoutMode') or d.get('layout_mode') or 'auto')[:20]}
    with conn() as c:
        c.execute('''INSERT INTO templates(id,name,format,line1,line2,line3,qr_mode,qr_template,created_at,updated_at,text_template,layout_mode)
        VALUES(:id,:name,:format,:line1,:line2,:line3,:qr_mode,:qr_template,:created_at,:updated_at,:text_template,:layout_mode)
        ON CONFLICT(id) DO UPDATE SET name=excluded.name,format=excluded.format,line1=excluded.line1,line2=excluded.line2,line3=excluded.line3,qr_mode=excluded.qr_mode,qr_template=excluded.qr_template,text_template=excluded.text_template,layout_mode=excluded.layout_mode,updated_at=excluded.updated_at''',row)
    return jsonify(row)
...
def export():
    with conn() as c:
        return jsonify(format='spoolman-niimbot-backup',version=2,exported_at=now(),templates=[dict(r) for r in c.execute('SELECT * FROM templates')],history=[dict(r) for r in c.execute('SELECT * FROM history')])

@app.route('/api/import',methods=['POST','OPTIONS'])
def import_backup():
    if request.method=='OPTIONS':return ('',204)
    d=request.get_json(force=True) or {}
    if d.get('format')!='spoolman-niimbot-backup' or int(d.get('version',0)) not in (1,2):return jsonify(error='Ungültiges Backupformat'),400
    backup_db('pre-restore')
    with conn() as c:
        c.execute('DELETE FROM templates'); c.execute('DELETE FROM history')
        for raw in d.get('templates',[]):
            t=now(); row={'id':str(raw.get('id') or f"tpl-{int(time.time()*1000)}"),'name':str(raw.get('name') or 'Vorlage')[:120],'format':str(raw.get('format') or '40x40')[:20],'line1':str(raw.get('line1') or '')[:500],'line2':str(raw.get('line2') or '')[:500],'line3':str(raw.get('line3') or '')[:500],'qr_mode':str(raw.get('qr_mode') or raw.get('qrMode') or 'url')[:30],'qr_template':str(raw.get('qr_template') or raw.get('qrTemplate') or '{{spool.url}}')[:1000],'created_at':raw.get('created_at') or t,'updated_at':raw.get('updated_at') or t,'text_template':str(raw.get('text_template') or raw.get('textTemplate') or '\n'.join(x for x in [raw.get('line1',''),raw.get('line2',''),raw.get('line3','')] if x))[:5000],'layout_mode':str(raw.get('layout_mode') or raw.get('layoutMode') or 'auto')[:20]}
            c.execute('''INSERT INTO templates(id,name,format,line1,line2,line3,qr_mode,qr_template,created_at,updated_at,text_template,layout_mode) VALUES(:id,:name,:format,:line1,:line2,:line3,:qr_mode,:qr_template,:created_at,:updated_at,:text_template,:layout_mode)''',row)
